...
import io.hyperstack4j.registry.QuantizationType;
...
class InferenceApiServerTest {
    ...
    static void startServer() {
        ...
        List<NodeDescriptor> nodes = List.of(
                node("n1", 4 * GB), node("n2", 4 * GB), node("n3", 4 * GB));

        ShardMap shardMap = ShardPlanner.create()
        ...
        ModelDescriptor tinyllama = ModelDescriptor.of(
                "tinyllama", "llama", TOTAL_LAYERS, HIDDEN_DIM, VOCAB_SIZE, NUM_HEADS,
    ...
    void streaming_inference_returns_200_with_event_stream_content_type() throws Exception {
        var request = HttpRequest.newBuilder()
                .uri(URI.create(BASE + "/inference/stream"))
    ...
    private static NodeDescriptor node(String id, long vram) {
        return new NodeDescriptor(id, "localhost", 9092,
                vram, vram, NodeStatus.READY, 1.0, Instant.now(), Instant.now());
    }
}