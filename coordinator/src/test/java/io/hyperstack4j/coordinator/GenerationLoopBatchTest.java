...
class GenerationLoopBatchTest {
    ...
    void batch_of_one_matches_single_generate() {
        ...
        List<GenerationResult> batchResults = batchLoop.generateBatch(
                List.of(new BatchEntry(req, TokenConsumer.discard())));
    ...
    void batch_returns_one_result_per_entry() {
        List<GenerationResult> results = loop.generateBatch(List.of(
                entry("prompt one"),
                entry("prompt two"),
                entry("prompt three")
        ));
    ...
    void eos_stops_individual_request_early_without_affecting_others() {
        ...
        InferenceRequest req1 = InferenceRequest.of("m",
                List.of(ChatMessage.user("stop early")),
                SamplingParams.defaults().withMaxTokens(5), RequestPriority.NORMAL);
        InferenceRequest req2 = InferenceRequest.of("m",
                List.of(ChatMessage.user("run full")),
        ...
        GenerationLoop earlyStopLoop = new GenerationLoop(
                tokenizer, Sampler.create(), earlyStopPipeline,
                new KVCacheManager(new GpuKVCache(64 * 1024 * 1024), new CpuKVCache(1000)));

        List<GenerationResult> results = earlyStopLoop.generateBatch(List.of(
                new BatchEntry(req1, TokenConsumer.discard()),
                new BatchEntry(req2, TokenConsumer.discard())
        ));
    ...
    void token_consumer_called_per_token_per_request() {
        ...
        BatchEntry e1 = new BatchEntry(
                InferenceRequest.of("m", List.of(ChatMessage.user("a")),
                        SamplingParams.defaults().withMaxTokens(3), RequestPriority.NORMAL), c1);
        BatchEntry e2 = new BatchEntry(
                InferenceRequest.of("m", List.of(ChatMessage.user("b")),
    ...
    void forwardBatch_called_once_per_step_not_per_request() {
        ...
        GenerationLoop batchLoop = new GenerationLoop(
                tokenizer, Sampler.create(), countingPipeline,
                new KVCacheManager(new GpuKVCache(64 * 1024 * 1024), new CpuKVCache(1000)));
