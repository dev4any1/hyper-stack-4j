...
class InferenceRequestTest {
    ...
    void high_priority_sorts_before_normal() {
        InferenceRequest high   = InferenceRequest.of("m", List.of(ChatMessage.user("hi")),
                SamplingParams.defaults(), RequestPriority.HIGH);
        InferenceRequest normal = InferenceRequest.of("m", List.of(ChatMessage.user("hi")),
    ...
    void same_priority_uses_fifo_ordering() throws InterruptedException {
        InferenceRequest first  = InferenceRequest.of("m", List.of(ChatMessage.user("a")),
                SamplingParams.defaults(), RequestPriority.NORMAL);
        Thread.sleep(5); // ensure different timestamps
        InferenceRequest second = InferenceRequest.of("m", List.of(ChatMessage.user("b")),
                SamplingParams.defaults(), RequestPriority.NORMAL);
    ...
    void rejects_blank_model_id() {
        assertThatThrownBy(() -> InferenceRequest.of("  ",
    ...
    void rejects_empty_messages() {
        assertThatThrownBy(() -> InferenceRequest.of("model",
                List.of(), SamplingParams.defaults(), RequestPriority.NORMAL))
                .isInstanceOf(IllegalArgumentException.class);
    }
}
