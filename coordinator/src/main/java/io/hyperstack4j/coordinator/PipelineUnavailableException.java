 ...
 *
 * Two scenarios:
 *   CIRCUIT_OPEN       — all node circuits were OPEN at call time (no attempt made)
 *   RETRIES_EXHAUSTED  — one or more nodes were tried but all threw exceptions
 *
 * The REST layer converts this to HTTP 503 with a Retry-After hint.
 * The scheduler may re-queue the request or complete the future exceptionally.
 */
public final class PipelineUnavailableException extends RuntimeException {

    public enum Reason {
        /** All node circuit breakers were OPEN — request rejected without attempting a forward pass. */
        CIRCUIT_OPEN,
    ...
    /** Whether the failure is potentially transient — worth retrying at the scheduler level. */
