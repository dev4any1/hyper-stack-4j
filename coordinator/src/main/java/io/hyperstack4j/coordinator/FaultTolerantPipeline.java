...
import io.hyperstack4j.health.CircuitBreaker;
 ...
 *
 * ── Routing ───────────────────────────────────────────────────────────────────
 * forward() scans the node list in order, skipping OPEN circuits. The first
 * permitted node that succeeds wins. On failure it records the failure, waits
 * backoffMs (if configured), then tries the next permitted node.
 *
 ...
 * Each NodePipeline carries its own CircuitBreaker. Transitions happen here:
 *   forward pass succeeds → recordSuccess()
 *   forward pass throws   → recordFailure() (may trip the circuit)
 *   VRAM_CRITICAL event   → forceOpen() immediately
 *   NODE_STALE event      → forceOpen() immediately
 *   NODE_RECOVERED event  → reset() — return to CLOSED
 *
 * ── Health Event Hooks ────────────────────────────────────────────────────────
 * HealthReactor calls these after evaluating HealthEvents from the probe:
 *   onVramCritical(nodeId)   — force-open that node's circuit
 *   onNodeStale(nodeId)      — force-open that node's circuit
 *   onNodeRecovered(nodeId)  — reset that node's circuit to CLOSED
 *
 * ── Thread Safety ─────────────────────────────────────────────────────────────
 * CircuitBreaker is already thread-safe (ReentrantLock inside).
 * The node list is immutable after construction.
 * The nodeIndex map is ConcurrentHashMap — safe for concurrent reads.
 */
public final class FaultTolerantPipeline implements InferencePipeline {
    ...
    /**
     * One pipeline replica + its circuit breaker.
     * In production: pipeline = gRPC stub to a real node JVM.
     * In tests:      pipeline = StubInferencePipeline or a throwing mock.
     */
    public record NodePipeline(
            String         nodeId,
    ...
    public FaultTolerantPipeline(List<NodePipeline> nodes, RetryPolicy retryPolicy) {
        ...
        for (NodePipeline n : nodes) nodeIndex.put(n.nodeId(), n);
    }
    ...
    public static FaultTolerantPipeline wrapping(String nodeId, InferencePipeline pipeline) {
        return new FaultTolerantPipeline(
                List.of(NodePipeline.of(nodeId, pipeline)),
                RetryPolicy.once()
        );
    }
    ...
    public float[] forward(String requestId, int[] tokens, int startPos) {
        ...
        for (NodePipeline node : nodes) {
            if (tried >= retryPolicy.maxAttempts()) break;
            if (!node.circuitBreaker().isCallPermitted()) continue;
            ...
            try {
                ...
                log.warning("forward() failed on node " + node.nodeId()
                        + " (attempt " + tried + "): " + e.getMessage());
        ...
        if (tried == 0) {
            throw new PipelineUnavailableException(
                    PipelineUnavailableException.Reason.CIRCUIT_OPEN, 0,
                    "all " + nodes.size() + " node circuit(s) are OPEN");
        }
        throw new PipelineUnavailableException(
                PipelineUnavailableException.Reason.RETRIES_EXHAUSTED, tried,
     ...
     *
     * Routes the entire batch to the first permitted node. On failure the
     * batch is retried on the next permitted node. This keeps all requests
     * in a batch on the same node for consistent KV cache behaviour.
     */
    @Override
    public float[][] forwardBatch(List<String>  requestIds,
                                  List<int[]>   allTokens,
        ...
        for (NodePipeline node : nodes) {
            ...
            try {
                ...
                log.warning("forwardBatch() failed on node " + node.nodeId()
                        + " (attempt " + tried + "): " + e.getMessage());
                node.circuitBreaker().recordFailure();
                lastCause = e;
                sleepBackoff();
    ...
    /**
     * Vocab size delegated to the first node.
     * All nodes in a cluster serve the same model — vocab size is identical.
     */
    @Override
    ...
    /**
     * Force-open this node's circuit immediately.
     * Called by HealthReactor on VRAM_CRITICAL events.
     * No-op if the nodeId is not in this pipeline's node list.
     */
    ...
    /**
     * Force-open this node's circuit immediately.
     * Called by HealthReactor on NODE_STALE events (missed health probes).
     * No-op if the nodeId is not in this pipeline's node list.
     */
    ...
    /**
     * Reset this node's circuit to CLOSED.
     * Called by HealthReactor on NODE_RECOVERED events.
     * No-op if the nodeId is not in this pipeline's node list.
     */
    ...
    private void sleepBackoff() {
        if (retryPolicy.backoffMs() <= 0) return;
        try {
