 ...
 *
 * maxAttempts:  total number of node attempts (1 = no retry, 2 = one retry, etc.)
 * backoffMs:    fixed delay between attempts (0 = immediate retry)
 *
 ...
 *
 * Presets:
 *   none()     — 1 attempt — call fails immediately if the pipeline throws
 *   once()     — 2 attempts, 50ms backoff — default for most requests
 *   aggressive() — 3 attempts, 100ms backoff — for HIGH priority requests
 */
