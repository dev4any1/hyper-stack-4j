 ...
 *
 * Routes:
 *   POST   /v1/inference           — blocking, returns full InferenceResponse
 *   POST   /v1/inference/stream    — SSE, streams TokenEvent per generated token
 *   GET    /v1/models              — list all registered models
 *   GET    /v1/models/{modelId}    — get model by ID
 *   DELETE /v1/models/{modelId}    — unload model
 *   GET    /v1/cluster/health      — cluster health overview
 *
 * Thread model:
 *   Javalin uses Virtual Threads (configured via VirtualThreadPool).
 *   Both blocking and SSE handlers call scheduler.submit() which dispatches
 *   generation on its own virtual thread. The SSE handler joins the future,
 *   blocking its virtual thread (cheap — no OS thread pinned).
 *
 * Error handling:
 *   400 — bad request (missing/empty messages)
 *   404 — model not found
 *   429 — scheduler queue full
...
public final class InferenceApiServer {
    ...
    public void start(int port) {
        ...
        app.exception(RequestScheduler.QueueFullException.class, (e, ctx) -> {
            ctx.status(429).json(Map.of(
                    ...
                    "retryAfterMs", e.retryAfterSeconds() * 1000
            ));
        });
    ...
    private void handleBlockingInference(Context ctx) {
        ...
        if (request == null) return; // parseRequest already set error response
    ...
    private void handleStreamingInference(Context ctx) {
        ...
        if (!modelRegistry.isLoaded(modelId)) {
            ctx.status(503).json(errorBody(503, "MODEL_NOT_LOADED",
        ...
        SseTokenConsumer consumer = new SseTokenConsumer(
                request.requestId(),
                data -> {
                    try {
    ...
    private void handleListModels(Context ctx) {
        ...
        ctx.json(Map.of(
                "models", models.stream().map(this::toModelResponse).toList(),
                "total",  models.size()
        ));
    }
    ...
    private void handleGetModel(Context ctx) {
        ...
        modelRegistry.getModel(modelId).ifPresentOrElse(
                m -> ctx.json(toModelResponse(m)),
                () -> ctx.status(404).json(errorBody(404, "NOT_FOUND",
    ...
    private ApiInferenceRequest parseBody(Context ctx) {
        try {
            ...
            ctx.status(400).json(errorBody(400, "BAD_REQUEST",
    ...
    private String resolveModelId(String requested) {
        ...
        return modelRegistry.listModels().stream()
                .filter(m -> modelRegistry.isLoaded(m.modelId()))
                .map(ModelDescriptor::modelId)
                .findFirst()
                .orElse(null);
    }

    private InferenceRequest toInferenceRequest(ApiInferenceRequest body, String modelId) {
        List<ChatMessage> messages = body.messages().stream()
                .map(m -> new ChatMessage(m.role(), m.content()))
                .toList();

        SamplingParams params = buildSamplingParams(body.sampling());
        RequestPriority priority = parsePriority(
                body.sampling() != null ? body.sampling().priority() : null);

        return InferenceRequest.of(modelId, messages, params, priority);
    }

    private SamplingParams buildSamplingParams(ApiSampling s) {
        if (s == null) return SamplingParams.defaults();
        SamplingParams params = SamplingParams.defaults();
        if (s.maxTokens()    != null) params = params.withMaxTokens(s.maxTokens());
        if (s.temperature()  != null) params = params.withTemperature(s.temperature());
        return params;
    }

    private RequestPriority parsePriority(String priority) {
        if (priority == null) return RequestPriority.NORMAL;
    ...
    private Map<String, Object> toModelResponse(ModelDescriptor m) {
        return Map.of(
                ...
                "quantization", m.quantization().displayName(),
                "totalLayers",  m.totalLayers(),
                "hiddenDim",    m.hiddenDim(),
                "vocabSize",    m.vocabSize(),
                "status",       m.status().name(),
                "estimatedVram", m.humanReadableSize()
        );
    }
    ...
    public record ApiInferenceRequest(
            String          requestId,
            String          modelId,
