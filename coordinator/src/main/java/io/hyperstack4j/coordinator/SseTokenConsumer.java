 ...
 *
 * Decoupled from Javalin via the SseEmitter functional interface —
 * in production pass client::sendEvent; in tests pass a List::add.
 *
 * Event format (one per generated token):
 *   {"requestId":"...","token":" world","tokenId":1917,"isComplete":false}
 *
...
public final class SseTokenConsumer implements TokenConsumer {
    ...
    /**
     * Send the terminal event with isComplete=true.
     * Called by the route handler after the generation future completes.
     *
    ...
    private String buildEvent(String token, int tokenId, boolean isComplete, String finishReason) {
        ...
        sb.append("{");
        appendString(sb, "requestId", requestId);    sb.append(",");
        appendString(sb, "token",     token);         sb.append(",");
        appendInt   (sb, "tokenId",   tokenId);       sb.append(",");
