 ...
 *
 * Problem: 16 clients all send the same 500-token system prompt.
 * Without prefix caching: each request recomputes those 500 tokens.
 * With prefix caching: first request computes + caches, rest skip forward to token 501.
 *
 * Usage in the inference loop:
 *   1. findLongestPrefix(tokens)  → PrefixMatch (matched length + KV block refs)
 *   2. If match.length > 0: start forward pass at match.length (skip matched prefix)
 *   3. After generation: cachePrefix(tokens, kvBlockRefs)
 *
...
public final class PrefixCache {
    ...
    public PrefixMatch findLongestPrefix(int[] tokens) {
        if (tokens == null || tokens.length == 0) return PrefixMatch.empty();
        ...
        try {
            ...
            for (int token : tokens) {
                ...
                current = next;
                matchLen++;
                if (current.cacheKey != null) lastCacheKey = current.cacheKey;
            }

            return matchLen > 0 && lastCacheKey != null
    ...
    public void cachePrefix(int[] tokens, int prefixLen, String cacheKey) {
        if (tokens == null || prefixLen < 1 || cacheKey == null) return;
