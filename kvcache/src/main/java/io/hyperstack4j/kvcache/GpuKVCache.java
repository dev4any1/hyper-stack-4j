 ...
 *
 * Current implementation: stores references in a ConcurrentHashMap index
 * (the actual CUDA buffer management is wired in by the node module which
 * has the JCuda dependency). This module stays free of the JCuda dep so
 * it can compile and test without a GPU present.
 *
 * The node module subclasses or wraps this with real CUDA buffer calls.
 * See: node/GpuKVCacheBackend.java
 *
...
public final class GpuKVCache implements KVCache {
    ...
    public void put(KVKey key, KVBlock block) {
        ...
        if (prev != null) usedBytes.addAndGet(-prev.sizeBytes());
    ...
    public void evict(String requestId) {
        index.keySet().stream()
                ...
                .forEach(k -> {
                    ...
                    if (removed != null) usedBytes.addAndGet(-removed.sizeBytes());
                });
    }
    ...
    @Override public long   estimatedSizeBytes() { return usedBytes.get(); }
    @Override public String tierName()           { return "gpu"; }

    public long vramBudgetBytes() { return vramBudgetBytes; }

    // ── Helpers ───────────────────────────────────────────────────────────────

    private void evictOldest() {
        index.entrySet().stream()
                .min((a, b) -> a.getValue().lastAccessedAt()
                        .compareTo(b.getValue().lastAccessedAt()))
                .ifPresent(entry -> {
