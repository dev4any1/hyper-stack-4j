 ...
 *
 * Behaviour: - Forks 3 node JVMs (same as ThreeNodeClusterIT / ClusterHarness)
 * - Wires GenerationLoop → ProcessPipelineClient → 3 × EmbeddedNodeServer -
 * Reads stdin line-by-line; each line is a prompt sent to the cluster - Tokens
 * stream to stdout as they arrive (no buffering) - Type exit or quit to shut
 * down cleanly - Ctrl-C triggers the shutdown hook and tears down all 3 node
 * JVMs
 *
 * Environment variables: DTYPE FLOAT32 | FLOAT16 | INT8 (default: FLOAT32)
 * MAX_TOKENS integer (default: 200) TEMPERATURE float (default: 0.7)
 *
 * OLLAMA_MODEL e.g. tinyllama or llama3 — when set, skips the stub gRPC cluster
 * entirely and streams real tokens from a local Ollama instance
 * (http://localhost:11434). No extra Maven dependency needed — uses
 * java.net.http.HttpClient (built-in since JDK 11). OLLAMA_URL override Ollama
 * base URL (default: http://localhost:11434)
 *
 * MODEL_PATH path to a GGUF model file — when set, uses CpuForwardPassHandler
 * on each node to run real transformer inference instead of stubs. e.g.
 * MODEL_PATH=/models/TinyLlama-1.1B-Chat-v1.0.Q4_K_M.gguf
 *
 * Launched by run-me.sh cluster via: mvn exec:java -pl integration \
...
public final class ConsoleMain {
	...
	static {
		...
		if (!verbose) {
			...
			try {
				ch.qos.logback.classic.Logger logbackRoot = (ch.qos.logback.classic.Logger) org.slf4j.LoggerFactory
						.getLogger(org.slf4j.Logger.ROOT_LOGGER_NAME);
				logbackRoot.setLevel(ch.qos.logback.classic.Level.DEBUG);
			} catch (Exception ignored) {
				/* logback not on classpath — safe to skip */ }
		}
	}
	...
	/**
	 * Real-model mode: bypass the stub cluster entirely. Streams tokens from a
	 * local Ollama instance using java.net.http.HttpClient (JDK built-in — zero
	 * extra Maven dependencies).
	 *
	 * Get started: curl -fsSL https://ollama.com/install.sh | sh ollama pull
	 * tinyllama # 637 MB — fast on CPU, good for dev ollama pull llama3.2 # 2 GB —
	 * much better quality ollama pull mistral # 4 GB — excellent quality
	 *
	 * Then run: OLLAMA_MODEL=tinyllama ./run-me.sh cluster OLLAMA_MODEL=llama3.2
	 * MAX_TOKENS=500 ./run-me.sh cluster
	 */
	private static void runOllamaRepl(String model, String baseUrl, int maxTokens, float temperature) throws Exception {
		print(CYAN + "▶ Ollama mode — model: " + BOLD + model + RESET + CYAN + "  url: " + baseUrl + RESET);

		HttpClient http = HttpClient.newHttpClient();

		// Verify Ollama is reachable before entering the REPL
		try {
			HttpRequest ping = HttpRequest.newBuilder().uri(URI.create(baseUrl + "/api/tags")).GET().build();
		...
		while (true) {
			...
			String body = "{\"model\":\"" + model + "\",\"stream\":true," + "\"options\":{\"num_predict\":" + maxTokens
					+ ",\"temperature\":" + temperature + "}," + "\"messages\":[" + String.join(",", history) + "]}";

			HttpRequest req = HttpRequest.newBuilder().uri(URI.create(baseUrl + "/api/chat"))
			...
			System.out.println();
			System.out.printf(DIM + "     [%d tokens · %d ms · %s]" + RESET + "%n", tokens, elapsed, model);
			System.out.println();
	...
	private static void runCpuClusterRepl(ActivationDtype dtype, int maxTokens, float temperature, String modelPath)
			throws Exception {
		...
		ProcessPipelineClient pipeline = new ProcessPipelineClient(harness.nodeAddresses(),
				EmbeddedNodeServer.VOCAB_SIZE, dtype);
		Tokenizer tokenizer;
		...
		while (true) {
			...
			InferenceRequest request = InferenceRequest.of("tinyllama", List.of(ChatMessage.user(line)), params,
			...
			boolean verbose = "true".equalsIgnoreCase(System.getProperty("HYPER_VERBOSE"));
			io.hyperstack4j.coordinator.TokenConsumer consumer = new io.hyperstack4j.coordinator.TokenConsumer() {
				@Override
	...
	private static float parseFloatVal(String val) {
		try {
			return Float.parseFloat(val.strip());
		} catch (NumberFormatException e) {
			return 0.7f;
		}
	}
	...
	/**
	 * Extract the string value of a JSON key from a flat JSON line. Handles:
	 * "key":"value" and "key": "value" and nested one level deep. Good enough for
	 * Ollama’s streaming protocol; not a general parser.
	 */
