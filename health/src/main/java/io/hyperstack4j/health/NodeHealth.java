...
/**
 * Immutable health snapshot for a single inference node.
 * Published by each node's GPU health probe every 5s.
 * Stored in Hazelcast IMap("node-health") keyed by nodeId.
 */
public record NodeHealth(
        ...
        long    vramFreeBytes,
        long    vramTotalBytes,
        double  temperatureCelsius,    // -1.0 if unavailable
