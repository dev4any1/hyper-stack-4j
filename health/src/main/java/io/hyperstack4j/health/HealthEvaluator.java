 ...
 * in production, direct calls in tests) and emits HealthEvents when
 * thresholds are crossed.
 *
 * State tracked:
 *   - Previous health level per node (to avoid duplicate events)
...
public final class HealthEvaluator {
    ...
    /**
     * Evaluate a fresh NodeHealth snapshot.
     * Returns zero or more HealthEvents representing state transitions.
     */
    ...
    private HealthLevel levelFor(NodeHealth health) {
        if (health.isVramCritical(thresholds.vramCritical())) return HealthLevel.CRITICAL;
        if (health.isVramWarning(thresholds.vramWarning()))   return HealthLevel.WARNING;
        return HealthLevel.OK;
    }

    private enum HealthLevel { OK, WARNING, CRITICAL }
}
