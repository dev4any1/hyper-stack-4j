...
public final class EmbeddedNodeServer {
	...
	private static final class NodeServiceImpl extends NodeServiceGrpc.NodeServiceImplBase {
		...
		@Override
		public void forwardPass(io.hyperstack4j.api.grpc.ForwardRequest request,
			...
			try {
				...
				if (context.hasEmbeddings()) {
					...
					intBuf.get(tokenIds);
					nodeReq = io.hyperstack4j.node.ForwardRequest.withTokens(request.getRequestId(), tokenIds,
					...
					nodeReq = io.hyperstack4j.node.ForwardRequest.withActivations(request.getRequestId(),
