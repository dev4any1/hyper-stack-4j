 ...
 *
 * For the first node (hasEmbeddings=true): activations is null, tokenIds is set.
 * For subsequent nodes: activations carries the float[] from the previous node.
 */
public record ForwardRequest(
        String  requestId,
        int[]   tokenIds,       // non-null for first node only
