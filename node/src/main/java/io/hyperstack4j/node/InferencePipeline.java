...
/**
 * Contract for executing a forward pass through a pipeline of transformer layers.
 *
 * Single-request: forward() — one request, one logit array.
 * Batched:        forwardBatch() — N requests, N logit arrays, ONE GPU call.
 *
 ...
 * GpuForwardPassHandler overrides forwardBatch() to use CUDA batched matrix ops,
 * turning N serial GPU launches into one, dramatically increasing utilisation.
 */
public interface InferencePipeline {
     ...
     * @param startPos  KV cache offset — tokens before this index are already cached
     ...
     *
     * Contract:
     *   - requestIds.size() == allTokens.size() == startPositions.size()
     *   - result[i] corresponds to requestIds.get(i)
     *   - All result arrays have length vocabSize()
     *
     ...
     */
    default float[][] forwardBatch(List<String> requestIds,
                                   List<int[]>  allTokens,
    ...
    /**
     * Vocabulary size — length of every logit array returned by forward/forwardBatch.
     */
    int vocabSize();
}