 ...
 *
 * Used exclusively at the gRPC transport boundary:
 *   - compress before writing {@code ForwardRequest.activation}
 *   - decompress after reading  {@code ForwardResponse.activation}
 *
 ...
 *
 * Wire layouts:
 *   FLOAT32 → big-endian IEEE 754 float, 4 bytes/element
 *   FLOAT16 → big-endian IEEE 754 half-precision, 2 bytes/element
 *   INT8    → [scale:float32 big-endian (4 bytes)] [quantised:signed byte × N]
 *
 * Thread-safe: all methods are stateless.
 */
public final class ActivationCodec {

    private ActivationCodec() {}   // utility class — no instances
    ...
    public static byte[] encode(float[] floats, ActivationDtype dtype) {
        if (floats == null || floats.length == 0) return new byte[0];
        return switch (dtype) {
    ...
    /**
     * Decode {@code bytes} back to a float[].
     * Must be called with the same dtype that was used for encoding.
     *
     * @param bytes encoded bytes (from {@link #encode}); null or empty returns float[0]
    ...
    /**
     * Encode to IEEE 754 half-precision (FP16).
     * 2 bytes per element, big-endian.
     */
    private static byte[] encodeFloat16(float[] floats) {
        ...
        for (float f : floats) buf.putShort(floatToHalf(f));
        return buf.array();
    }
    ...
    private static float[] decodeFloat16(byte[] bytes) {
        ...
        for (int i = 0; i < out.length; i++) out[i] = halfToFloat(buf.getShort());
        return out;
    }
     ...
     *
     * Bit layout (float32):  s[1] e[8] m[23]
     * Bit layout (float16):  s[1] e[5] m[10]
     *
    ...
    static short floatToHalf(float f) {
        ...
        if (exp <= 0) {
            ...
            if (exp < -10) return (short) sign;          // too small → ±0
    ...
    static float halfToFloat(short half) {
        ...
        if (exp == 0) {
            if (mant == 0) return Float.intBitsToFloat(sign);  // ±0
     ...
     *
     * Wire layout: {@code [scale:float32 big-endian (4 bytes)][quantised:signed byte × N]}.
     *
     * Symmetric (not asymmetric) quantisation is used to keep the zero point at 0,
     * which preserves true-zero activations and simplifies reconstruction.
     * Using 127 (not 128) as the max avoids asymmetry of signed byte range.
     */
    ...
    private static float[] decodeInt8(byte[] bytes) {
        ...
        float[] out = new float[bytes.length - 4];
        for (int i = 0; i < out.length; i++) out[i] = buf.get() * scale;
        return out;
    }
}