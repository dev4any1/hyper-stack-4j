 ...
 *   F16  — half-precision → float32 (IEEE 754 bit manipulation, no JNI)
 *   BF16 — bfloat16 → float32
 *   Q8_0 — 8-bit symmetric, block size 32
 *   Q4_0 — 4-bit symmetric, block size 32
 *   Q4_K — 4-bit with per-superblock scale/min, block size 256  (Q4_K_M uses this)
 *   Q6_K — 6-bit with per-superblock scale, block size 256
 *
 * Thread-safe after construction — tensor data is loaded on demand then cached.
 *
 * Usage:
 *   GgufReader r = GgufReader.open(Path.of("/models/TinyLlama.Q4_K_M.gguf"));
 *   LlamaConfig cfg = LlamaConfig.from(r);
 *   float[] w = r.tensor("blk.0.attn_q.weight");
 */
public final class GgufReader implements AutoCloseable {
    ...
    private GgufReader(FileChannel channel, Map<String, Object> metadata,
    ...
    public static GgufReader open(Path file) throws IOException {
        ...
        for (long i = 0; i < tensorCount; i++) {
            ...
            for (int d = 0; d < ndims; d++) dims[d] = readUInt64(channel, pos);
            int      type   = readInt32(channel, pos);
            long     offset = readUInt64(channel, pos);
            long     nelems = 1; for (long d : dims) nelems *= d;
    ...
    public Object meta(String key) { return metadata.get(key); }

    public String  metaString(String key)           { Object v = metadata.get(key); return v instanceof String s ? s : null; }
    public long    metaLong(String key, long def)   { Object v = metadata.get(key); return v instanceof Number n ? n.longValue() : def; }
    public int     metaInt(String key, int def)     { Object v = metadata.get(key); return v instanceof Number n ? n.intValue()  : def; }
    public float   metaFloat(String key, float def) { Object v = metadata.get(key); return v instanceof Number n ? n.floatValue(): def; }
    public boolean hasTensor(String name)            { return tensors.containsKey(name); }
    public Map<String, Object> allMetadata()         { return java.util.Collections.unmodifiableMap(metadata); }

    /**
     * Load and dequantize a tensor to float[].
    ...
    public float[] tensor(String name) throws IOException {
        ...
        if (info == null)
            throw new IllegalArgumentException("Tensor not found: " + name
                    + "  (available: " + tensors.size() + " tensors)");
    ...
    /** Extract 6-bit scale[j] from Q4_K scales block (12 bytes, 8 scales + 8 mins each 6-bit). */
    private static int getScale4K(byte[] sc, int j) {
        if (j < 4) return sc[j] & 0x3F;
    ...
    //   [ql:128 bytes][qh:64 bytes][scales:16 bytes][d:f16] = 210 bytes per 256 elements
    ...
    private static int readInt32(FileChannel ch, long[] pos) throws IOException {
        ...
        ch.read(buf, pos[0]); pos[0] += 4; buf.flip();
        return buf.getInt();
    }

    private static long readUInt64(FileChannel ch, long[] pos) throws IOException {
        ByteBuffer buf = ByteBuffer.allocate(8).order(ByteOrder.LITTLE_ENDIAN);
        ch.read(buf, pos[0]); pos[0] += 8; buf.flip();
        return buf.getLong();
    }

    private static Object readMetadataValue(FileChannel ch, long[] pos, int vtype)
            throws IOException {
        return switch (vtype) {
            case GGUF_METADATA_VALUE_TYPE_UINT8,
                 GGUF_METADATA_VALUE_TYPE_INT8  -> { ByteBuffer b = ByteBuffer.allocate(1); ch.read(b, pos[0]); pos[0]++; b.flip(); yield (int) b.get(); }
            case GGUF_METADATA_VALUE_TYPE_UINT16,
                 GGUF_METADATA_VALUE_TYPE_INT16 -> { ByteBuffer b = ByteBuffer.allocate(2).order(ByteOrder.LITTLE_ENDIAN); ch.read(b, pos[0]); pos[0] += 2; b.flip(); yield (int) b.getShort(); }
            case GGUF_METADATA_VALUE_TYPE_UINT32,
                 GGUF_METADATA_VALUE_TYPE_INT32 -> { int v = readInt32(ch, pos); yield v; }
            case GGUF_METADATA_VALUE_TYPE_FLOAT32 -> { ByteBuffer b = ByteBuffer.allocate(4).order(ByteOrder.LITTLE_ENDIAN); ch.read(b, pos[0]); pos[0] += 4; b.flip(); yield b.getFloat(); }
            case GGUF_METADATA_VALUE_TYPE_BOOL  -> { ByteBuffer b = ByteBuffer.allocate(1); ch.read(b, pos[0]); pos[0]++; b.flip(); yield b.get() != 0; }
            case GGUF_METADATA_VALUE_TYPE_STRING -> readString(ch, pos);
            case GGUF_METADATA_VALUE_TYPE_UINT64,
                 GGUF_METADATA_VALUE_TYPE_INT64  -> readUInt64(ch, pos);
            case GGUF_METADATA_VALUE_TYPE_FLOAT64 -> { ByteBuffer b = ByteBuffer.allocate(8).order(ByteOrder.LITTLE_ENDIAN); ch.read(b, pos[0]); pos[0] += 8; b.flip(); yield b.getDouble(); }
