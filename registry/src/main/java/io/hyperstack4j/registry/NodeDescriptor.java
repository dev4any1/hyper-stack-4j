...
public record NodeDescriptor(
        String    nodeId,          // unique node identifier e.g. "node-192.168.1.10-0"
        ...
        NodeStatus status,
        double    seedScore,       // IMQ-inspired election score (updated by registry)
    ...
    public double vramPressure() {
        if (vramTotalBytes == 0) return 0.0;
    ...
    public NodeDescriptor withStatus(NodeStatus status) {
        return new NodeDescriptor(nodeId, host, grpcPort, vramTotalBytes, vramFreeBytes,
                status, seedScore, registeredAt, lastSeenAt);
    }
