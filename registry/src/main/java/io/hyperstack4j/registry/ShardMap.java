...
/**
 * Complete layer-to-node assignment for a loaded model.
 * Stored in Hazelcast IMap("shard-maps") keyed by modelId.
 *
 * Assignments are ordered by startLayer ascending — this is the
 * pipeline execution order: coordinator sends activations to
 * assignments.get(0), then .get(1), etc.
 *
 * Recomputed by the registry whenever cluster membership changes.
 */
public record ShardMap(
        String               modelId,
        int                  totalLayers,
        ...
        for (ShardAssignment a : assignments) {
            if (a.startLayer() != expected) {
                throw new IllegalStateException(
                        "Layer gap or overlap at layer " + expected
                        + " — assignment starts at " + a.startLayer());
            }
            expected = a.endLayer();
        }
        if (expected != totalLayers) {
            throw new IllegalStateException(
                    "ShardMap covers " + expected + " layers but model has " + totalLayers);
        }
    }
}
