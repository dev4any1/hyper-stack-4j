 ...
 *
 * Given a list of healthy nodes and model metadata, computes
 * the optimal ShardMap — contiguous layer blocks, VRAM-aware,
 * highest-scoring nodes get first pick.
 *
 * Algorithm:
 *   1. Filter nodes: only IDLE or READY
 *   2. Sort by seedScore descending
 *   3. For each node: assign as many layers as fit in usableVram
 *   4. If all layers are not assigned: throw InsufficientClusterVramException
 *
...
public final class ShardPlanner {
     ...
     * @param nodes            all registered nodes (any status — filtered internally)
     * @return a validated ShardMap ready for use
     * @throws InsufficientClusterVramException if cluster has insufficient VRAM
     */
    public ShardMap plan(String modelId, int totalLayers,
                         long vramPerLayerBytes, List<NodeDescriptor> nodes) {
        ...
        if (eligible.isEmpty())
            throw new InsufficientClusterVramException(
                    "No eligible nodes available for model: " + modelId);
        ...
        if (currentLayer < totalLayers) {
            throw new InsufficientClusterVramException(String.format(
