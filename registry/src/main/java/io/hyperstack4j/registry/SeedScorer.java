 ...
 *
 * Scores each node to determine shard assignment priority.
 * Higher score = gets assigned first = gets more layers.
 *
 * Formula:
 *   score = w1 * vramScore
 *         + w2 * stabilityScore
 *         + w3 * connectivityScore
 *
 * Where:
 *   vramScore        = vramFree / vramTotal          (0.0 → 1.0)
 *   stabilityScore   = 1.0 if READY, 0.5 if IDLE, 0.0 otherwise
 *   connectivityScore = provided by caller (0.0 → 1.0)
 *                       typically: 1.0 - (latencyMs / maxLatencyMs)
 *
...
public final class SeedScorer {
    ...
    public double score(NodeDescriptor node, double connectivityScore) {
        ...
        return wVram * vramScore
             + wStability * stabilityScore
             + wConnectivity * connScore;
    }

    // ── Component scorers ─────────────────────────────────────────────────────

    double vramScore(NodeDescriptor node) {
        if (node.vramTotalBytes() == 0) return 0.0;
    ...
    public double wVram()         { return wVram; }
    public double wStability()    { return wStability; }
    public double wConnectivity() { return wConnectivity; }
}
