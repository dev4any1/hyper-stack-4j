 ...
 *
 * Transitions:
 *   UNLOADED → LOADING  (register() called — shards being sent to nodes)
 *   LOADING  → LOADED   (all nodes confirm shard is ready)
 *   LOADING  → ERROR    (any node fails to load its shard)
 *   LOADED   → UNLOADED (unregister() called)
