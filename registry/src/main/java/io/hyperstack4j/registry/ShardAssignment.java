...
/**
 * Assignment of a contiguous layer range to a specific node.
 * Part of a ShardMap — one ShardAssignment per node in the pipeline.
 *
 * Pipeline order is determined by startLayer ascending.
 * The node with the lowest startLayer holds the embedding table.
 * The node with the highest endLayer holds the output projection.
 */
public record ShardAssignment(
        String  nodeId,
        String  host,
        int     grpcPort,
        int     startLayer,          // inclusive
