...
class NodeDescriptorTest {
    ...
    private NodeDescriptor node(long vramTotal, long vramFree, NodeStatus status) {
        return new NodeDescriptor("n1", "192.168.1.10", 9092,
                vramTotal, vramFree, status, 0.8, Instant.now(), Instant.now());
    }
    ...
    void rejects_invalid_grpc_port() {
        assertThatThrownBy(() -> new NodeDescriptor(
                "n1", "host", 99999, 1, 1, NodeStatus.IDLE, 0, Instant.now(), Instant.now()))
                .isInstanceOf(IllegalArgumentException.class);
    }

    @Test
    void rejects_vram_free_exceeding_total() {
        assertThatThrownBy(() -> node(1_000L, 2_000L, NodeStatus.IDLE))
                .isInstanceOf(IllegalArgumentException.class);
    }
}
