...
class ShardMapTest {
    ...
    private ShardAssignment shard(String nodeId, int start, int end) {
        return new ShardAssignment(nodeId, "host", 9092, start, end,
                start == 0, end == 32);
    }
    ...
    void validate_coverage_passes_for_valid_map() {
        assertThatCode(() -> validMap().validateCoverage())
                .doesNotThrowAnyException();
    }

    @Test
    void validate_coverage_detects_gap() {
        ShardMap gapped = new ShardMap("model", 32, List.of(
                shard("n1", 0, 10),
                shard("n2", 12, 32)  // gap at layers 10-11
        ), Instant.now());
        assertThatThrownBy(gapped::validateCoverage)
    ...
    void validate_coverage_detects_incomplete_coverage() {
        ShardMap incomplete = new ShardMap("model", 32, List.of(
                shard("n1", 0, 16)   // only covers half
        ), Instant.now());
        assertThatThrownBy(incomplete::validateCoverage)
                .isInstanceOf(IllegalStateException.class);
    }
