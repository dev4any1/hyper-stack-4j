 ...
 *
 * Encoding algorithm (SentencePiece BPE):
 *   1. Prepend ▁ to the first word, ▁ to each subsequent word (space normalisation)
 *   2. Initialise: one symbol per UTF-8 character (fall back to byte tokens for OOV)
 *   3. Greedily merge the adjacent pair with the highest score until no merges remain
 *
...
public final class GgufTokenizer implements Tokenizer {
    ...
    public static GgufTokenizer load(GgufReader r) {
        ...
        log.info("Tokenizer loaded: vocabSize=" + V
                + " bos=" + bosId + " eos=" + eosId);
        return new GgufTokenizer(vocab, scores, tokenTypes, bosId, eosId, padId, unkId);
    }

    private GgufTokenizer(String[] vocab, float[] scores, int[] tokenTypes,
                          int bosId, int eosId, int padId, int unkId) {
        this.vocab      = vocab;
        this.scores     = scores;
        ...
        for (int i = 0; i < vocab.length; i++) pieceToId.put(vocab[i], i);
    }
    ...
    public int[] encode(String text) {
        if (text == null || text.isEmpty()) return new int[]{bosId};
        ...
        result[0] = bosId;
        for (int i = 0; i < syms.size(); i++) result[i + 1] = syms.get(i).id;
        return result;
    }
    ...
    public String decode(int[] tokenIds) {
        ...
        for (int id : tokenIds) sb.append(decodeToken(id));
        return sb.toString().replace(SP, ' ').stripLeading();
    }
    ...
    public String decodeToken(int tokenId) {
        ...
        if (type == 3 /* control */ || tokenId == bosId || tokenId == eosId) return "";
    ...
    @Override public int    vocabSize()  { return vocab.length; }
    @Override public String modelType()  { return "llama"; }
