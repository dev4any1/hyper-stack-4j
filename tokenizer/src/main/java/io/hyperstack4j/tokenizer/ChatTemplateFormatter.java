...
/**
 * Applies a ChatTemplate to a list of messages and returns the formatted prompt.
 *
 * Sits between the coordinator's InferenceRequest and the Tokenizer.encode() call:
 *
 *   InferenceRequest.messages()
 *       → ChatTemplateFormatter.format()
 *       → prompt String
 *       → Tokenizer.encode()
 *       → int[] tokens
...
public final class ChatTemplateFormatter {
    ...
    /**
     * Convenience factory — resolve by model type string.
     ...
     *
     * @param messages ordered list of conversation turns (system, user, assistant...)
