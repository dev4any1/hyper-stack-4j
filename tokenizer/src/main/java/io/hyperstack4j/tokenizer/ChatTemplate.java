...
/**
 * Formats a list of ChatMessages into a single prompt string
 * in the format expected by a specific model family.
 *
 * Each model family has different special tokens and structure.
 * Templates are stateless — one instance shared across all requests.
 *
 * Supported families:
...
public interface ChatTemplate {
    ...
    /**
     * Format a list of messages into a model-ready prompt string.
     * The returned string is passed directly to Tokenizer.encode().
     */
    ...
    static ChatTemplate llama3() {
        return new ChatTemplate() {
            ...
            @Override public String modelType() { return "llama3"; }
        };
    }

    /**
     * Mistral chat template.
     * [INST] {user} [/INST] {assistant}</s>[INST] ...
     * System message prepended to first user turn.
     */
    static ChatTemplate mistral() {
        return new ChatTemplate() {
            ...
            @Override public String modelType() { return "mistral"; }
        };
    }
    ...
    static ChatTemplate gemma() {
        return new ChatTemplate() {
            ...
            @Override public String modelType() { return "gemma"; }
        };
    }
    ...
    static ChatTemplate chatml() {
        return new ChatTemplate() {
            ...
            @Override public String modelType() { return "chatml"; }
        };
    }
    ...
    /**
     * Resolve a template by model type string.
     * Falls back to ChatML for unknown model types.
     */
    static ChatTemplate forModelType(String modelType) {
        if (modelType == null) return chatml();
